...
void Bag::Add_Item(const string& name, int quantity, const string& category) {
    ...
    items[name] = { items[name].first + quantity, category }; // 如果物品已存在则增加数量
    cout << "添加物品: " << name << ", 数量: " << quantity << ", 类别: " << category << endl;
}
...
void Bag::Use_Item(const string& name, int quantity) {
    ...
    cout << "使用物品: " << name << ", 数量: " << quantity << endl;

    // 如果使用后数量为0，则移除此物品
    if (items[name].first == 0) {
        items.erase(name);
        cout << "物品: " << name << " 已被移除！" << endl;
    }
}
